...
[дополнение после аудита](../verification/activity-v0-audit-followup.md).
...
администратора. Права проверяются через общий Gateway client без кеша; group,
name и timezone могут кешироваться до 30 секунд. Frontend `is_admin` не учитывается.
