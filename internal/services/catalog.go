...
func Components() []Component {
	components := []Component{
		{Code: Activity, Version: "v1", MigrationOwner: "activity", Dependencies: []string{CRMEvents, "gateway", "core-policy"}, Modes: []string{"embedded", "grpc"}, Requirement: "activity capability and installation pilot; current server-confirmed admin for all v0 endpoints", MaxConnections: 3, Health: "/ready", HTTPRoutes: []apicontract.Route{apicontract.ActivityPanel, apicontract.ActivitySettings, apicontract.ActivityConfigure}, RPCs: []string{"GetPanel", "GetSettings", "Configure", "OperationStatus"}},
		{Code: CRMEvents, Version: "v1", MigrationOwner: "crm-events", Dependencies: []string{"gateway", "core-policy"}, Modes: []string{"embedded", "grpc"}, Requirement: "current source and consumer admission", MaxConnections: 5, Concurrency: 2, Health: "/ready", HTTPRoutes: []apicontract.Route{apicontract.ActivityStatus, apicontract.ActivitySync, apicontract.ActivityOperation}, RPCs: []string{"Apply", "QueryEvents", "Status", "OperationStatus"}},
		{Code: "gateway", Version: "v1", MigrationOwner: "core", Dependencies: []string{"core-policy", "oauth"}, Modes: []string{"embedded", "grpc"}, Requirement: "one active outbound budget owner; shares Core worker pool; bootstrap account restricted to Core identity", MaxConnections: 6, Health: "/ready", RPCs: []string{"Events", "Users", "CoreBootstrap.GetAccount"}},
	}
